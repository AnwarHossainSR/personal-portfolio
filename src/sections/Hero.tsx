...
export function Hero() {
	return (
		...
		>
			<h1 className="hero-rise hero-rise-1 max-w-[16ch] font-display text-[2.75rem] font-medium leading-[1.04] tracking-tight text-ink sm:text-6xl md:text-7xl">
			...
			<div className="hero-rise hero-rise-2 mt-8 max-w-2xl space-y-5 text-lg leading-relaxed text-muted">
				{profile.pitch.map((paragraph) => (
					<p key={paragraph}>{paragraph}</p>
				))}
			</div>

			<div className="hero-rise hero-rise-3 mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm">
				<a
					href="#contact"
